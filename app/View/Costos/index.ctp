      ...
      <?php foreach ($costos as $c): ?>
        <tr>
          ...
          <td><a href="javascript:"
                 onclick="cargarmodal('<?php echo $this->Html->url(array('action' => 'usuario', $c['Costo']['id'])) ?>');"
                 class="btn btn-default btn-sm btn-icon icon-left"><i class="entypo-pencil"></i>Editar</a>
            <?php echo $this->Html->link('<i class="entypo-cancel"></i>Eliminar', array('action' => 'delete', $c['Costo']['id']), array('class' => 'btn btn-danger btn-sm btn-icon icon-left', 'escape' => false, 'confirm' => 'Esta seguro de eliminar al usuario ' . $c['Costo']['id'])); ?>
          </td>
        </tr>
      <?php endforeach; ?>
      </tbody>
