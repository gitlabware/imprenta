        ...
        <div class="col-md-12">
            <div class="form-group">
                <select name="data[Costo][insumo_id]" class="form-control" data-allow-clear="true" data-placeholder="Select one city...">
                    ...
                    <?php $seleccionado='';
                    if($i['Insumo']['id']==$this->request->data['Costo']['insumo_id']){
                        $seleccionado='selected';
                    }
                    ?>
                    <option <?php echo $seleccionado;?> value="<?php echo $i['Insumo']['id']; ?>"><?php echo $i['Insumo']['nombre']; ?> (<?php echo $i['Insumo']['descripcion']; ?>)</option>
                    <?php endforeach; ?>
                </select>
            </div>
        </div>
